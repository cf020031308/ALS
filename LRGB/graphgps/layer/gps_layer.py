...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        elif local_gnn_type == 'ALS':
            self.local_model = MAPR(
                dim_h, dim_h, dropout=dropout, edge_dim=dim_h,
    ...
    def forward(self, batch):
        ...
        if self.local_model is not None:
            ...
            h_local = h_local + h # Residual connection.
        ...
        if self.has_global:
            ...
            if self.global_model_type == 'Transformer':
                h_global = self._gl_block(h_dense, None, ~mask)[mask]
            elif self.global_model_type == 'BiasedTransformer':
                # Use Graphormer-like conditioning, requires `batch.attn_bias`.
                h_global = self._gl_block(h_dense, batch.attn_bias, ~mask)[mask]
            elif self.global_model_type == 'Performer':
                h_global = self.self_attn(h_dense, mask=mask)[mask]
            elif self.global_model_type == 'BigBird':
                h_global = self.self_attn(h_dense, attention_mask=mask)
            else:
                raise RuntimeError(f"Unexpected {self.global_model_type}")

            h_global = self.dropout_global(h_global)
            h_global = h_global + h # Residual connection.
            if self.layer_norm:
